...
class SNMPWalk(ServiceScan):
	...
	async def run(self, service):
		...
		await service.execute('snmpwalk -c public -v 1 {address} NET-SNMP-EXTEND-MIB::nsExtendObjects', outfile='{protocol}_{port}_snmp_snmpwalk_nsExtendObjects.txt')
		await service.execute('snmpwalk -c public -v 1 {address} NET-SNMP-EXTEND-MIB::nsExtendOutputFull', outfile='{protocol}_{port}_snmp_snmpwalk_nsExtendOutputFull.txt')
